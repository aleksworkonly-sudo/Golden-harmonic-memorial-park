...
/* ---------- Velaris: animated WebGL gradient background (adapted, no build step needed) ---------- */
function Velaris({ bg = '#f6f1e8', colors = ['#e6d9b8', '#d9c48f', '#8fae9c', '#2f5d4c'], speed = 0.5, grain = 0.12, height = '100%', style }) {
  ...
  useEffect(() => {
    ...
    if (!gl) return; // silently no-op on very old browsers, page still works fine
    ...
    const fragmentShaderGLSL = `
      ...
      void main() {
        ...
        col = mix(col, u_colors[0], smoothstep(-0.2, 0.5, n1) * 0.6);
        col = mix(col, u_colors[1], smoothstep(-0.1, 0.6, n2) * 0.5);
        col = mix(col, u_colors[2], smoothstep(-0.3, 0.4, n3) * 0.4);
        col = mix(col, u_colors[3], smoothstep(0.0, 0.7, n1 * n2) * 0.15);
        float glow = smoothstep(0.8, 0.0, dist) * 0.15;
        col += u_colors[1] * glow;
        col = mix(col * 0.9, col, vignette);
        float grainV = fract(sin(dot(uv, vec2(12.9898, 78.233))) * 43758.5453 + u_time);
        col += (grainV - 0.5) * u_grain * 0.06;
  ...
  return (
    <div ref={containerRef} style={{ position: 'absolute', inset: 0, height, width: '100%', overflow: 'hidden', ...style }}>
      <canvas ref={canvasRef} style={{ pointerEvents: 'none', position: 'absolute', inset: 0, width: '100%', height: '100%' }} />
    </div>
  );
}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700, fontSize: 12.5, padding: '7px 12px', height: 32,
              boxShadow: '0 2px 8px rgba(0,0,0,.15)',
              lineHeight: 1.15, textAlign: 'center', whiteSpace: 'normal'
            }}
            onClick={() => { addToCart(tier, qty); setQty(1); }}>
            {inCartItem ?
            <span style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <span style={{ fontSize: 12 }}>✓ In cart ({inCartItem.qty})</span>
                <span style={{ fontSize: 10, fontWeight: 500, opacity: 0.85 }}>Add {qty} more</span>
              </span> :
